...
pacman::p_load(dplyr, ggplot2, DESeq2, tidyverse, forcats, readr, usedist, gridExtra)
...
```{r}
sample_names <- c('mock_1', 'mock_2', 'mock_3', 'mutant_1', 'mutant_2', 'mutant_3', 'reovirus_1', 'reovirus_2', 'reovirus_3')
```
...
tree_non_norm <-
  hclust(data_dist, method = "complete")
plot(tree_non_norm, xlab = "Sample", main = NULL)
...
```{r normal tree}
norm_dist <- dist(t(norm_data), method = "euclidean")
norm_dist <- dist_setNames(data_dist, sample_names)

tree_norm <-
  hclust(norm_dist, method = "complete")
  plot(tree_norm, xlab = "Sample", main = NULL)
...
```{r color by treatment}
plot(PCA$x[ ,1], PCA$x[ , 2], pch = 19, col = colors, main = "PCA (colored by treatment)", xlab = "PCA
1", ylab = "PCA2", ylim = c(-150,150))
legend("topright", legend = c("mock", "mutant", 'reovirus'), pch = 16, col = colors, bty = "n")
```
