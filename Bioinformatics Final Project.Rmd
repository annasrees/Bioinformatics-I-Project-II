...
```
The p-value given is 0.2935. There is **not** a significant difference between the total number of counts by treatment group.
...
## Distribution per Sample boxplots
...
non_normal_graph
```
We can see from the boxplot above that there is slight variation in counts per gene
...
```
Above is a distance tree demonstrating similarity between samples based on the read counts.

Below, we conduct simple normalization by generating correction factors based on the means.
...
We then apply the correlation factor and adjust accordingly.
```{r normalizing}
...
## Cluster dendogram (distance tree) of normalized data
```{r normal tree}
data_dist <- dist(high_abun_log2, method = "euclidean")
data_dist <- dist_setNames(data_dist, sample_names)
