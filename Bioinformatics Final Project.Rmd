...
ggplot(data = sum_by_sample |> group_by(type) |> summarize('sum' = sum(sum)),
       mapping = aes(x = sum, 
                     y = type,
                     fill = type)) +
...
```
putting words in the middle because it's funky
```{r t test}
...
high_abundance_genes <-
  data |>
  filter(if_all('Donor 97 pDC':'Donor 36 SLAN DC', ~ !.x  %in% 0))
...
ggplot(data = data_long,
  ...
  geom_boxplot(data = sum_by_sample,
           mapping = aes(y = sum,
                         fill = type),
           color = 'black') +
